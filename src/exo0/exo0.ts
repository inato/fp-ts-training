...
export const isOddP: (value: number) => boolean = function (value: number) {
  return pipe(value, isEven, not);
};

export const isOddF: (value: number) => boolean = function (value: number) {
  return flow(isEven, not)(value);
};
...
export const next: (value: number) => number = function (value: number) {
  return pipe(
    isEven(value),
    ifThenElse(
      () => value / 2,
...
export const next3: (value: number) => number = function (value: number) {
  return flow(next, next, next)(value);
};
