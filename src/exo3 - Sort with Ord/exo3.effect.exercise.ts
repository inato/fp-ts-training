...
import { Option, Array, String, Number, Order, pipe } from 'effect';
// We're actually implementing the functions, so we don't need unimplemented
// import { unimplemented } from '../utils.effect';
...
export const sortNumbersDescending = (numbers: ReadonlyArray<number>): ReadonlyArray<number> => 
  pipe(numbers, Array.sort(Order.reverse(Number.Order)));
...
export const sortOptionalNumbers = (
  ...
  pipe(optionalNumbers, Array.sort(Option.getOrder(Number.Order)));
...
const byName = Order.mapInput(String.Order, (person: Person) => person.name);

export const sortPersonsByName = (persons: ReadonlyArray<Person>): ReadonlyArray<Person> => 
  pipe(persons, Array.sort(byName));
