// Effect training Exercise 7
// Manipulate collections with type-classes

import { HashMap, HashSet, pipe, Array, Order } from 'effect';
...
//
// HINT:
// - We can create our HashMap with a simpler reduce approach
...
export const mapWithConcatenatedEntries: HashMap.HashMap<number, string> = pipe(
  ...
  Array.reduce(
    ...
    (map, [key, value]) => {
      ...
      const newValue = existing._tag === "Some" ? existing.value + value : value;
      return HashMap.set(map, key, newValue);
    }
  )
);
